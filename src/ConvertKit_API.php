...
class ConvertKit_API
{
    ...
    public function get_sequence_subscriptions($sequence_id, $sort_order = 'asc')
    {
        ...
        $this->create_log(sprintf("GET sequence subscriptions: %s, %s, %s", $request, json_encode($options),
            $sequence_id));
    ...
    public function add_subscriber_to_sequence($sequence_id, $email)
    {
        ...
        $this->create_log(sprintf("POST add subscriber to sequence: %s, %s, %s, %s", $request, json_encode($options),
            $sequence_id, $email));
